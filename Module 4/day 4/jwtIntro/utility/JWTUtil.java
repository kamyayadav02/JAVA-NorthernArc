package org.northernarc.jwtdemo.utility;
...
//it validates the token, matches the token with userdetails and the extract username if it matches and not expired then its accepted.
        public boolean validateToken(String token, UserDetails userDetails) {
            final String username = extractUsername(token);
            return (username.equals(userDetails.getUsername()) && !isTokenExpired(token));
        }
        //if all correct then create upat token this authToken is saved in securityContextHolder
